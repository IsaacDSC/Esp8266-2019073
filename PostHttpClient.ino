...
void setup() {
  ...
  digitalWrite(LED_BUILTIN, HIGH);//inicia o led interno ligado
  pinMode(D1, OUTPUT);//led do RFID
  pinMode(D4, OUTPUT);//led do alerta
  digitalWrite(D4, LOW);//INICIA COM ELE LIGADO
  pinMode(D2, OUTPUT);//rele do motor
  digitalWrite(D2, HIGH);//INICIA COM ELE DESLIGADO
...
void loop() {
    ...
    // Procura por cartao RFID
      ...
      if (conteudo.substring(1) == "74 71 53 73" || "E2 BA E0 2E") //UID 1 - Cartao
      {
        ...
        delay(5000);              // DELAY /espera 3 segundos       
  ...
  client.flush();
      ...
      if(req.indexOf("alerta") != -1){
        Serial.println("ALERTA SEM EPI");
        alert();  
        digitalWrite(D4, LOW);
        delay(500);          
      }else{
